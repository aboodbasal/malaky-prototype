  ...
  return (
    <>
      ...
      <g stroke="#f3ede6" strokeOpacity="0.14" strokeWidth="1">
        {rows.map((y) =>
          cols.slice(0, -1).map((x) => <line key={`h${x}-${y}`} x1={x} y1={y} x2={x + 60} y2={y} />),
        )}
        {cols.map((x) =>
          rows.slice(0, -1).map((y) => <line key={`v${x}-${y}`} x1={x} y1={y} x2={x} y2={y + 60} />),
        )}
      </g>
...
function SignalFlow() {
  ...
  return (
    <>
      ...
      <line x1="292" y1="208" x2="380" y2="208" stroke="#ff4e2d" strokeOpacity="0.5" strokeWidth="2" strokeDasharray="5 6" />
      <circle cx="292" cy="208" r="9" fill="#ff4e2d" opacity="0.95" />
      <circle cx="292" cy="208" r="19" fill="none" stroke="#ff4e2d" strokeOpacity="0.35" strokeWidth="1.5" />
